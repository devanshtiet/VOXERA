...
export function policyToPrompt(d: PolicyDirectives): string {
  const parts = [
    ...
    d.escalate !== "none"
      ? "Escalation required: offer to bring in a real person from the team to take over. Say it exactly the " +
        "way a friend would offer to grab someone — e.g. \"Let me grab someone from the team for you\" or " +
